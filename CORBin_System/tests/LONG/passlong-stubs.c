...
CORBA_long
passlong_foo(passlong _obj, const CORBA_long x, CORBA_Environment * ev)
{
   ...
   if (_obj->servant && _obj->vepv && passlong__classid) {
      _ORBIT_retval =
	 ((POA_passlong__epv *) _obj->vepv[passlong__classid])->foo(_obj->
								    servant,
								    x, ev);
